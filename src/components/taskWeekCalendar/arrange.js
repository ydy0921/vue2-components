...
function arrangeColumn (group, task) {
	...
	group.push([task])
	// change the weight of the existed task if need
	for (let i = 0; i < group.length - 1; i++) {
		for (const oldTask of group[i]) {
			if (oldTask.w === group.length - 1 - i && !testTimeOverlap(oldTask, [task.s, task.e])) oldTask.w++
		}
	}
}
